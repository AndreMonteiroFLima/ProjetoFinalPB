...
public class InvoiceDaoImpl implements InvoiceDao {
    ...
    public void update(InvoiceModel invoice) {
        try (Connection connection = databaseService.getConnection()) {
            String sql = "UPDATE invoice SET invoice_date = ?, invoice_value = ?, client_id = ? WHERE invoice_id = ?";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setTimestamp(1, new Timestamp( invoice.getInvoiceDate().getTime() ));
                ps.setBigDecimal(2, invoice.getInvoiceValue());
                ps.setInt(3, invoice.getClientId());
                ...
                invoice.getInvoiceItens().forEach(item -> {
                    updateInvoiceProduct(invoice.getInvoiceId(), item);
                    if(getInvoicesProducts(invoice.getInvoiceId()).size() < invoice.getInvoiceItens().size()){
                        if(!existsInvoiceProduct(invoice.getInvoiceId(), item))
                            insertInvoiceProduct(invoice.getInvoiceId(), item);
                    }
                });
            } catch (Exception e) {
                throw new RuntimeException("Error while updating invoice: " + e.getMessage());
            }
        } catch (SQLException e) {
            throw new RuntimeException("Error while trying to connect to database: " + e.getMessage());
        }
    ...
    private void insertInvoiceProduct(int invoiceId, Integer item) {
        try (Connection connection = databaseService.getConnection()) {
            String sql = "INSERT INTO invoice_product (invoice_id, product_id) VALUES (?,?)";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setInt(1, invoiceId);
                ...
                throw new RuntimeException("Error while inserting invoice product: " + e.getMessage());
            }
        } catch (SQLException e) {
            throw new RuntimeException("Error while trying to connect to database: " + e.getMessage());
        }
    ...
    private boolean existsInvoiceProduct(int invoiceId, Integer item) {
        try (Connection connection = databaseService.getConnection()) {
            String sql = "SELECT * FROM invoice_product WHERE invoice_id = ? AND product_id = ?";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setInt(1, invoiceId);
                ps.setInt(2, item);
                ResultSet rs = ps.executeQuery();
                ...
                throw new RuntimeException("Error while checking if exists invoice product: " + e.getMessage());
            }
        } catch (SQLException e) {
            throw new RuntimeException("Error while trying to connect to database: " + e.getMessage());
        }
    ...
    private void updateInvoiceProduct(int invoiceId, Integer item) {
        try (Connection connection = databaseService.getConnection()) {
            String sql = "UPDATE invoice_product SET product_id = ?, invoice_id = ? WHERE invoice_id = ? AND product_id = ?";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setInt(1, item);
